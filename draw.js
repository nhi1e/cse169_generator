// --- draw.js---
let WATER_THICKNESS_MULT = 4.0; // increase for bolder acrylic strokes
...
// === Arc-stroke painter used by lilies and flowers ===
function drawFlowerSlice(_x, _y, _dir, _length, _thickness, _fromC, _toC) {
	const strokeCount = max(3, int(_length / 1.8));
	...
	for (let i = 0; i < strokeCount; i++) {
		...
		// bell-ish thickness, slightly broken (scumble)
		let nowThickness =
			_thickness * (t < 0.5 ? easeOutSine(t * 2) : easeOutSine((1 - t) * 2));
		nowThickness *= random(0.85, 1.15);

		let nowC = NYLerpColor(_fromC, _toC, t);
		nowC.slightRandomize(8, 10, 12);
		...
		push();
		translate(_x + random(-0.4, 0.4), _y + random(-0.4, 0.4));
		// “broken color” look with multiple arcs per step
		for (let k = 0; k < 2; k++) {
			const jitter = random(-6, 6);
			arc(
				offsetX + jitter * 0.1,
				offsetY + jitter * 0.1,
				nowThickness,
...
function drawLilyFlower(pg, x, y, palette, sizeScale = 1.0) {
	const rot = random(360);
	const baseLen = random(22, 46) * sizeScale;

	const cIn = palette.flowerInsideColor.copy();
	const cOut = palette.flowerOutsideColor.copy();
	const pA = palette.pistilColorA.copy();
	const pB = palette.pistilColorB.copy();

	// OUTER RING
	{
		const count = int(random(9, 14));
		for (let i = 0; i < count; i++) {
			const t = i / (count - 1);
			const deg = rot + lerp(-85, 85, t) + random(-6, 6);
			const len = baseLen * random(1.1, 1.5);
			const width = len * random(0.45, 0.75);
			const fromC = cOut.copy();
			fromC.slightRandomize(6, 8, 8);
			const toC = cIn.copy();
			toC.slightRandomize(6, 8, 8);
			drawPetalStroke(tempBG, x, y, deg, len, width, fromC, toC);
		}
	}

	// MIDDLE RING
	{
		const count = int(random(7, 11));
		for (let i = 0; i < count; i++) {
			const t = i / (count - 1);
			const deg = rot + lerp(-65, 65, t) + random(-5, 5);
			const len = baseLen * random(0.9, 1.2);
			const width = len * random(0.35, 0.55);
			const fromC = cOut.copy();
			fromC.slightRandomize(5, 6, 8);
			const toC = cIn.copy();
			toC.slightRandomize(5, 6, 8);
			drawPetalStroke(tempBG, x, y, deg, len, width, fromC, toC);
		}
	}
	...
	// PISTIL
	{
		const rays = int(random(6, 12));
		for (let i = 0; i < rays; i++) {
			const deg = rot + random(-40, 40);
			const len = baseLen * random(0.6, 1.0);
			const fromC = pA.copy();
			fromC.a = 0.6;
			const toC = pB.copy();
			toC.a = 0.3;
			drawNoiseLine(x + random(-3, 3), y + random(-2, 2), deg, len, fromC, toC);
		}
	}

	// INNER RING
	{
		const count = int(random(6, 10));
		for (let i = 0; i < count; i++) {
			const t = i / (count - 1);
			const deg = rot + lerp(-55, 55, t) + random(-5, 5);
			const len = baseLen * random(0.5, 0.9);
			const width = len * random(0.42, 0.65);
			const fromC = cOut.copy();
			fromC.slightRandomize(6, 8, 8);
			const toC = cOut.copy();
			toC.slightRandomize(6, 8, 8);
			drawPetalStroke(tempBG, x, y + 2, deg, len, width, fromC, toC);
		}
	}

	// SOFT CENTER GLOW
	{
		const glow = NYLerpColor(pA, pB, random(0.4, 0.8));
		noStroke();
		fill(glow.h, glow.s * 0.75, glow.b, 0.35);
		ellipse(x, y, baseLen * 0.6, baseLen * 0.45);
	}
}
